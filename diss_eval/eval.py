...
def eval(data, dmatrix, labels):
    ...
    return "$" + str(np.round(ch,4)) + "$ & $" + str(np.round(ch_adj,4)) + "$ & $" + str(np.round(ss,4)) + "$ & $" + str(np.round(db,4)) + "$ & $" + str(np.round(db_adj,4)) + "$ & $" + str(size[0]) + "/" + str(size[1]) + "$ & $" + str(len(set(labels))) + "$ \\\\"
...
print(label_kmeans)
mz = dframe.columns
label_kmeans2 = [x for _,x in sorted(zip(mz, label_kmeans))]
assign_mol_to_mz(label_kmeans2, dframe)
